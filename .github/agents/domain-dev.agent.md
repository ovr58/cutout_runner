...
- **Размер ответа.** Маска растягивается к **точным** W×H исходного кадра; вызывающий
  проверяет размер по самому файлу и отвергает несовпадение как отказ.
- **RGB ответа обязан совпадать с исходником пиксель в пиксель.**
